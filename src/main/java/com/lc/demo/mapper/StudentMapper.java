...
public interface StudentMapper {
    ...
    @Select("select * from student_view")
    ...
    @Select("explain select * from student where  stu_id=#{stuId} ")
    Student selectStudentByStuId(String stuId);

    @Delete("DELETE from student force index (idx_student_id) where stu_id=#{stuId}")
